...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		...
		* {margin: 0;padding: 0;font-family:Helvetica, Arial, Tahoma, sans-serif;}
		...
		.calendario th {border:1px dotted #ccc;background:#666;color:white;padding:10px 5px; text-align:center;font-size:16px;}
		.calendario td{padding:4px 6px 0 6px;text-align:center;border:1px dotted #ccc;width:100px;white-space:pre-line;}
		.calendario td p{margin:2px 0 2px 0;font-size:12px;border-bottom:1px solid #ccc;text-align:left;padding:0px}
		...
		.calendario input.enviar{border:1px dotted #ccc;background:white;width:110px !important;background:#282828;margin:0 0 0 10px;}
	...
	<div id="agenda">
		<?php
			...
			if (isset($_GET["borrarevento"]))
			{
				...
				if ($r1=pg_query($pgconn,$q1)) $mostrar="<p class='ok' id='mensaje'>Evento eliminado correctamente.</p>";
			...
			if (isset($_POST["addevent"])=="Si")
			{
				...
				if ($r1=pg_query($mysqlconn,$q1)) { $mostrar="<p class='ok' id='mensaje'>Evento guardado correctamente.</p>";
				echo('<script>location.href = "hoy.php";</script>');}
				else $mostrar="<p class='error' id='mensaje'>Se ha producido un error guardando el evento.</p>";
			}
*/
			if (!isset($_GET["fecha"]))
...
/*
	1 traer fecha actual
	2 traer ultimo dia del mes 
	3 query select * from tcalendario where fecha between **date con primer dial del mes** and **date con ultimo dial del mes**
*/
			...
			for ($i=1;$i<=$diasdespues;$i++)
			{
				...
				if ($i>=$primeromes && $i<=$tope)
				{
					...
					/*$revent=pg_query($mysqlconn,$sqlevent);
